...
interface FloatingAlertsWidgetProps {
  roadAlert?: string;
  onOpenBookings: () => void;
  onOpenWeatherSecurity?: () => void;
}

export const FloatingAlertsWidget: React.FC<FloatingAlertsWidgetProps> = ({
  roadAlert,
  onOpenBookings,
  ...
  const handleActionClick = (notif: AppNotification) => {
    ...
    if (notif.data?.linkAction === 'open_bookings' || notif.data?.linkAction === 'open_custom_requests') {
      onOpenBookings();
    } else if (notif.data?.linkAction === 'open_weather' && onOpenWeatherSecurity) {
      onOpenWeatherSecurity();
    }
  };
  ...
  return (
    <>
      {/* 1. SLEEK GLASSMORPHIC FLOATING WIDGET DOCKED ON RIGHT SIDE */}
      <div className="fixed top-20 sm:top-24 right-3 sm:right-5 z-40 animate-fadeIn">
        <div className="relative">
          ...
          >
            ...
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-white/40 via-transparent to-amber-500/10 dark:from-white/5 dark:to-amber-500/10 pointer-events-none"></div>

            {/* Pulsing Live Alert Indicator */}
            <span className="absolute -top-1 -right-1 flex h-4 w-4 z-10">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-4 w-4 bg-amber-500 border-2 border-white dark:border-slate-900 items-center justify-center text-[9px] font-extrabold text-white">
                {unreadCount > 0 ? (unreadCount > 9 ? '9+' : unreadCount) : '!'}
              </span>
            </span>

            {/* Icon Container with Bell & Pulse */}
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-amber-100 to-amber-200/70 dark:from-slate-800 dark:to-amber-950/80 flex items-center justify-center font-extrabold shadow-inner border border-white/60 dark:border-white/10 shrink-0 text-amber-700 dark:text-amber-300">
              <Bell className="w-5 h-5" />
            </div>

            {/* Details Content in Floating Card */}
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between gap-1">
                <span className="text-xs font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-1">
                  <span>Alerts & Passes</span>
                </span>
                <span className="text-[10px] font-extrabold text-amber-800 dark:text-amber-300 bg-amber-100/90 dark:bg-amber-950/80 px-2 py-0.5 rounded-lg border border-amber-200 dark:border-amber-800 font-mono shadow-xs">
                  {unreadCount} new
                </span>
              </div>

              <div className="flex items-center gap-1 mt-0.5 text-[10px] text-slate-600 dark:text-slate-300 font-medium truncate">
                {roadAlert ? (
                  <span className="truncate text-amber-900 dark:text-amber-200 font-bold">
                    ⚠️ {roadAlert}
                  </span>
                ) : (
                  <span className="truncate">Live Dispatch & Pass Updates</span>
                )}
              </div>

              <div className="flex items-center justify-between text-[9px] text-slate-400 dark:text-slate-400 mt-0.5">
                <span className="font-bold text-pine-700 dark:text-amber-400 uppercase tracking-wide">Spiti • Manali</span>
                <span className="text-pine-700 dark:text-amber-400 font-extrabold flex items-center group-hover:translate-x-0.5 transition-transform">
                  View Hub <ChevronRight className="w-2.5 h-2.5" />
                </span>
              </div>
            </div>
          </button>
          ...
          {isOpen && (
            <div className="absolute right-0 mt-3 w-80 sm:w-96 rounded-3xl bg-white dark:bg-slatehimachal-950 text-slate-900 dark:text-white shadow-2xl border border-slate-200 dark:border-slate-800 z-50 overflow-hidden animate-fadeIn backdrop-blur-xl">
              ...
              {/* Road Alert Banner if Available */}
              {roadAlert && (
                <div 
                  onClick={() => {
                    if (onOpenWeatherSecurity) onOpenWeatherSecurity();
                    setIsOpen(false);
                  }}
                  className="px-3.5 py-2.5 bg-amber-500/15 border-b border-amber-500/30 text-amber-900 dark:text-amber-200 flex items-center justify-between text-xs cursor-pointer hover:bg-amber-500/25 transition-colors"
                >
                  ...
                  <span className="text-[10px] font-extrabold text-amber-800 dark:text-amber-300 shrink-0 flex items-center gap-0.5">
                    Policy <ArrowRight className="w-3 h-3" />
                  </span>
                </div>
              )}
